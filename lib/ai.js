...
exports.semanticAnalysis = semanticAnalysis;
exports.generateInterviewQuestion = generateInterviewQuestion;
exports.analyzeAnswer = analyzeAnswer;
...
function analyzeJobDescription(jobDescription) {
    ...
    const lemmatizedWords = words.map((word) => {
        return (0, compromise_1.default)(word).nouns().toSingular().out("text") || (0, compromise_1.default)(word).verbs().toInfinitive().out("text") || word;
    });
    // Détection du type de poste (management ou technique)
    let type = "technical";
    const doc = (0, compromise_1.default)(cleanedDescription);
 ...
 */
function generateInterviewQuestion(jobDescription) {
    ...
    for (const question of questions) {
        // on check le type
        if (jobInfo.type === "management" && question.type !== "behavioral")
            continue;
        if (jobInfo.type === "technical" && question.type !== "technical" && question.type !== "general")
            continue;
        // On check le level
        if (jobInfo.level === "junior" && question.level === "senior")
            continue;
        if (jobInfo.level === "senior" && question.level === "junior")
            continue;
        ...
        if (jobInfo.technologies.length > 0 && !technologyFound)
            continue;
 ...
 */
function rateAnswer(answer, question, jobDescription) {
    // Analyse de la description du poste
    const jobInfo = analyzeJobDescription(jobDescription);
    const lowerCaseAnswer = answer.toLowerCase();
    ...
    const questionWords = cleanedQuestion.split(/\s+/);
    // On utilise la librairie compromise pour la lemmatisation  
    const docQuestion = (0, compromise_1.default)(cleanedQuestion);
    ...
    //Confiance
    if (answerKeywords.includes("non") ||
 ...
 */
function startConversation(jobDescription) {
 ...
 */
function getNextQuestion(conversationId) {
    ...
    let weakPointsQuestions = availableQuestions.filter(q => {
        for (const weakPoint of weakPoints) {
            if (q.keywords.includes(weakPoint))
                return true;
        }
        return false;
    });
     ...
     */
    export function addAnswer(conversationId, answer) {
        const conversation = conversations.find((c) => c.conversationId === conversationId);
        if (!conversation) {
            throw new Error(`Conversation with ID ${conversationId} not found.`);
        }
        const lastExchange = conversation.history[conversation.history.length - 1];
        if (!lastExchange || !lastExchange.question)
            return [];
        const jobDescription = conversation.jobDescription;
        ...
        weakPoints.forEach(weakPoint => {
            if (!conversation.weakPoints.includes(weakPoint))
                conversation.weakPoints.push(weakPoint);
        });
        return feedbacks;
    }
    /**
     ...
     */
    export function getAverageNote(conversationId) {
        const conversation = conversations.find((c) => c.conversationId === conversationId);
        if (!conversation) {
    ...
    */
    export function generateCoverLetter(cv_1, jobDescription_1) {
        return __awaiter(this, arguments, void 0, function* (cv, jobDescription, language = 'fr', tone = 'professional') {
            try {
          ...
          Description du poste: ${jobDescription}`,
                ...
                trackError(error, { feature: 'cover-letter-generation' });
                throw error;
            }
        });
    }
