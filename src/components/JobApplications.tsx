...
function JobApplications() {
  ...
  const deleteApplication = async (id: string) => {
    ...
    try {
      ...
      trackEvent('application_deleted', { user_id: user?.id, application_id: id })
    } catch (error) {
  ...
  const handleExportPDF = async (application: JobApplication) => {
    ...
    try {
      trackEvent('pdf_export_started', { user_id: user?.id, application_id: application.id });
      await downloadApplicationPDF(application);
      trackEvent('pdf_export_success', { user_id: user?.id, application_id: application.id });
    } catch (error) {
      console.error('Error exporting application to PDF:', error);
      trackEvent('pdf_export_failed', { user_id: user?.id, application_id: application.id, error: (error as Error).message });
    }
  };
