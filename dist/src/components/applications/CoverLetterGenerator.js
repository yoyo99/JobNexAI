...
import { decryptAES } from '../../lib/cryptoUtils';
...
export function CoverLetterGenerator() {
    ...
    const [iaEngine, setIaEngine] = useState('openai');
    const [apiKeySource, setApiKeySource] = useState('fallback');
    useEffect(() => {
        if (user) {
            loadJobs();
            loadCV();
                ...
                try {
                    ...
                    if (apiKeys[selectedEngine]) {
                        ...
                        try {
                            ...
                            apiKeyDecrypted = yield decryptAES((user === null || user === void 0 ? void 0 : user.id) || 'default', apiKeys[selectedEngine], apiKeys[selectedEngine + '_iv'] || '');
                            source = 'user';
                        }
                        catch (_a) {
                            apiKeyDecrypted = '';
