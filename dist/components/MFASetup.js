...
export function MFASetup({ onComplete, onCancel }) {
    ...
    const [smsFactorId, setSmsFactorId] = useState(null);
    const [smsChallengeId, setSmsChallengeId] = useState(null);
    const setupTOTP = () => __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            if (error || !data)
                throw error || new Error('No TOTP data');
            ...
            const { data: challengeData, error: challengeError } = yield supabase.auth.mfa.challenge({
                factorId: data.id
            });
            if (challengeError || !challengeData)
                throw challengeError || new Error('No challenge data');
    ...
    const setupSMS = () => __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            if (error || !data)
                throw error || new Error('No SMS data');
